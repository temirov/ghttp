...
func main() {
	...
	if userConfigErr != nil {
		logger.Fatal("resolve user config directory", zap.Error(userConfigErr))
	}
	...
	rootCmd := newRootCommand(resources)
	baseContext := context.WithValue(context.Background(), contextKeyApplicationResources, resources)
	rootCmd.SetContext(baseContext)

	if execErr := rootCmd.Execute(); execErr != nil {
		logger.Error("command execution failed", zap.Error(execErr))
		os.Exit(1)
	}
}
